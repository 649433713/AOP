...
/**
 * @author yinywf
 * @implNote ExcelHead与ExcelCell选择一个使用即可
...
public class ExcelUtil {
    ...
    public static <T> List<T> readExcelToEntity(Class<T> classzz, InputStream in, String fileName, List<ExcelHead> excelHeads ,int sheetNum) throws Exception {
        checkFile(fileName);    //是否EXCEL文件
        Workbook workbook = getWorkBoot(in, fileName); //兼容新老版本
        List<T> excelForBeans = readExcel(classzz, workbook.getSheetAt(sheetNum), excelHeads);  //解析Excel
        return excelForBeans;
    }
    ...
    public static <T> List<T> readExcelToEntity(Class<T> classzz, InputStream in, String fileName ,int sheetNum) throws Exception {
        return readExcelToEntity(classzz, in, fileName, null, sheetNum);
    }
    ...
    private static <T> List<T> readExcel(Class<T> classzz, Workbook workbook, List<ExcelHead> excelHeads) throws Exception {
        ...
        for (int sheetIndex = 0; sheetIndex < sheetNum; sheetIndex++) {
            ...
            beans.addAll(readExcel(classzz, sheet, excelHeads));
        }
        return beans;
    }

    private static <T> List<T> readExcel(Class<T> classzz, Sheet sheet, List<ExcelHead> excelHeads) throws Exception {
        List<T> beans = new ArrayList<T>();
            ...
            for (int cellIndex = firstCellNum; cellIndex < lastCellNum; cellIndex++) {
                ...
                for (Field field : fields) {
                    if (headName.equalsIgnoreCase(field.getName())
                            || (field.getAnnotation(ExcelCell.class) != null && headName.equalsIgnoreCase(field.getAnnotation(ExcelCell.class).value()))) {
    ...
    private static boolean isDateFied(Field field) {
        return (Date.class == field.getType()) || (Timestamp.class == field.getType()) ;
    }
    /**
     * 空值校验
     *
